...
std::tuple<UniTensor, std::vector<int>, std::vector<int>> partial_trace(UniTensor &A,
                                                                        std::vector<int> A_label) {
  ...
  if (num_cont > 0) {
    ...
    for (int i = 0; i < 2; i++)
      for (int j = 0; j < dup_list.size(); j++) cont_ind.push_back((cytnx_uint64)dup_list[j][i]);
    ...
    auto Ashape = A.shape();
    int cont_dim = 1;
    for (int i = 0; i < num_cont; i++) cont_dim *= Ashape[cont_ind[i]];
    ...
    int free_dim_prod = 1;
    for (int i = 0; i < free_dim.size(); i++) free_dim_prod *= free_dim[i];
    auto B = zeros({free_dim_prod});  // B is Tensor

    Tensor Aten = A.get_block();
    Aten =
      Aten.permute(vec_concatenate(free_ind, cont_ind)).reshape(free_dim_prod, cont_dim, cont_dim);
    for (int ip = 0; ip < cont_dim; ip++) {
      B = B + Aten(":", ip, ip);
    }
    // outlabel = B_label;
    return std::make_tuple<UniTensor, std::vector<int>, std::vector<int>>(
      UniTensor(B.reshape(free_dim), 0), {}, {});
  } else {
    // outlabel = A_label;
    Tensor Aten = A.get_block();
...
int check_inputs(std::vector<std::vector<int>> connect_list, std::vector<int> flat_connect,
                 std::vector<int> dims_list, std::vector<cytnx_int64> cont_order) {
  return 0;
}

UniTensor ncon(const std::vector<UniTensor> &tensor_list_in,
               const std::vector<std::vector<int>> &connect_list_in,
               const bool check_network /*false*/, std::vector<int> cont_order /*{}*/,
               const std::vector<cytnx_int64> &out_labels /*{}*/) {
  UniTensor out;
  ...
  if (cont_order.size() == 0) {
    ...
    for (int i = 0; i < flat_connect.size(); i++) {
      if (flat_connect[i] > 0) positive.push_back(flat_connect[i]);
    }
  ...
  while (cont_order.size() > 0) {
    ...
    for (int ele = 0; ele < connect_list.size(); ele++) {
      ...
      if (sum > 0) {
        locs.push_back(ele);
        std::cout << "locs: ";
        for (int j = 0; j < locs.size(); j++) std::cout << locs[j] << ' ';
        std::cout << '\n';
        std::cout << "connect_list: ";
        for (int j = 0; j < connect_list[ele].size(); j++) std::cout << connect_list[ele][j] << ' ';
        std::cout << std::endl;
        assert(sum == 1);
      }
      assert(sum <= 2);
    ...
    // cont_many, A_cont, B_cont = vec_intersect(connect_list[locs[0]], connect_list[locs[1]], true,
    // true);
    ...
    std::cout << "cout_many: ";
    for (int i = 0; i < cont_many.size(); i++) std::cout << (int)cont_many[i] << ' ';
    std::cout << '\n';
    std::cout << "A_cont: ";
    for (int i = 0; i < A_cont.size(); i++) std::cout << (int)A_cont[i] << ' ';
    std::cout << '\n';
    std::cout << "A_shape: ";
    for (int i = 0; i < tensor_list[locs[0]].shape().size(); i++)
      std::cout << (int)tensor_list[locs[0]].shape()[i] << ' ';
    std::cout << '\n';
    std::cout << "B_cont: ";
    for (int i = 0; i < B_cont.size(); i++) std::cout << (int)B_cont[i] << ' ';
    std::cout << std::endl;
    std::cout << "B_shape: ";
    for (int i = 0; i < tensor_list[locs[1]].shape().size(); i++)
      std::cout << (int)tensor_list[locs[1]].shape()[i] << ' ';
    std::cout << std::endl;
    ...
    for (cytnx_int64 i = alabel.back() + 1;
         i < alabel.back() + 1 + tensor_list[locs[1]].labels().size(); i++)
      blabel.push_back(i);
    for (cytnx_int64 i = 0; i < A_cont.size(); i++) {
      std::cout << "A_cont[" << i << "]: " << A_cont[i] << '\n';
      std::cout << "blabel.back()+i+1: " << blabel.back() + i + 1 << '\n';
      std::cout << "alabelINSIDE: ";
      for (int j = 0; j < alabel.size(); j++) std::cout << (int)alabel[j] << ' ';
      std::cout << std::endl;
    ...
    std::cout << "alabel: ";
    for (int i = 0; i < alabel.size(); i++) std::cout << (int)alabel[i] << ' ';
    std::cout << '\n';
    std::cout << "blabel: ";
    for (int i = 0; i < blabel.size(); i++) std::cout << (int)blabel[i] << ' ';
    std::cout << std::endl;

    UniTensor Ta = tensor_list[locs[0]].relabels(alabel);
    UniTensor Tb = tensor_list[locs[1]].relabels(blabel);

    // tensor_list.push_back(tensordot(tensor_list[locs[0]], tensor_list[locs[1]], axes=(A_cont,
    // B_cont))); std::cout<<Ta.labels()<<std::endl; std::cout<<Tb.labels()<<std::endl;
    ...
    connect_list.push_back(vec_concatenate(vec_erase(connect_list[locs[0]], A_cont),
                                           vec_erase(connect_list[locs[1]], B_cont)));

    // remove contracted tensors from list and update cont_order
    // cont_order = np.delete(cont_order,np.intersect1d(cont_order,cont_many, assume_unique=True,
  ...
  while (tensor_list.size() > 1) {
    std::vector<cytnx_int64> alabel;
    std::vector<cytnx_int64> blabel;
    for (int i = 0; i < tensor_list[tensor_list.size() - 2].labels().size(); i++)
      alabel.push_back(i);
    for (int i = alabel.back() + 1;
         i < alabel.back() + 1 + tensor_list[tensor_list.size() - 1].labels().size(); i++)
      blabel.push_back(i);
    ...
    tensor_list[tensor_list.size() - 2] =
      Contract(tensor_list[tensor_list.size() - 2], tensor_list[tensor_list.size() - 1]);
    connect_list[connect_list.size() - 2] =
      vec_concatenate(connect_list[connect_list.size() - 2], connect_list[connect_list.size() - 1]);
