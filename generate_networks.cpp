...
int main(void) {//ios_base::sync_with_stdio(0);cin.tie(0);cout.tie(0);
	...
	int cont_cnt=rnd(tot_bond_cnt/2),neg_bond_cnt=tot_bond_cnt-2*cont_cnt;
	...
	rep(i,0,SZ(links)){
		rep(j,0,SZ(links[i])){
			if(links[i][j]>0 and pos[links[i][j]]!=0){
				links[i][pos[links[i][j]]]*=-1;
				links[i][j]*=-1;
			}else if(links[i][j]>0 and pos[links[i][j]]==0)pos[links[i][j]]=j;
		}
		memset(&pos[0],0,sizeof(pos[0])*SZ(pos));
	}
